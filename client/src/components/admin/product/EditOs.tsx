...
export default function EditOs() {
  ...
  console.log(`EditOs render count: ${renderCount.current}`);

  const { id } = useParams();
  const navigate = useNavigate();

  const { fetchOs, updateOs } = useProductOsStore();
  ...
  const [os, setOs] = useState<AdminOsResponse | null>(null);
  ...
  // Fetch and set initial data when first load or refresh signal: os
  useEffect(() => {
    const handleFetchSetInitial = async (): Promise<void> => {
      ...
      try {
        ...
        const fetchedOs = await fetchOs(id);

        setOs(fetchedOs);

        const copiedOs = structuredClone(fetchedOs); // Avoid direct mutation
        setFormData({
          name: { val: copiedOs.name },
          logoUrl: { val: copiedOs.logoUrl },
          description: { val: copiedOs.description || "" },
        });
        setLogoPreviewUrl(copiedOs.logoUrl || defaultLogo);
      } catch (error) {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
      ...
      if (!os) {
        toast.error("Product OS data is not available.");
        return;
      }
      if (!canEditOs) {
        toast.error(
          "You do not have permission to update product OS information.",
        );
        return;
      }
      ...
      if (await validateForm()) {
        const getChangedData = async (): Promise<ProductOsUpdate> => {
          const changedData: ProductOsUpdate = {};

          if (formData.name.val !== os.name) {
            changedData.name = formData.name.val;
          }
          if (formData.description.val !== (os.description || "")) {
          ...
          } else if (formData.logoUrl.val === null && os.logoUrl) {
    ...
    [os, canEditOs, formData, process.isProcessing, updateOs],
  );
  ...
  return (
    <>
      {process.isInitializing ? (
        ...
        <ApiError errorMessage="Product OS data not found." />
      ) : (
        <>
          <Title
            title={`Update OS #ID ${os.id}`}
            ...
            <div className="col-lg-8">
              ...
              <div className="card shadow-sm mb-4">
                ...
                <div className="card-body">
                  <form onSubmit={handleSubmit} id="editOsForm">
                    {/* Name */}
                    <div className="mb-3">
                    ...
                    <div className="mb-3">
                      ...
                      <Textarea
                        ...
                        name="description"
                        className="form-control"
                        placeholder={os.description || "None"}
                        rows={4}
                  ...
                  <div className="row">
                    ...
                    <div className="col-md-6 mb-3">
                      ...
                      <input
                        ...
                        id="createdAt"
                        className="form-control"
                        value={new Date(os.createdAt).toLocaleString()}
                        disabled
                      />
                    </div>
                    <div className="col-md-6 mb-3">
                      ...
                      <input
                        ...
                        id="updatedAt"
                        className="form-control"
                        value={new Date(os.updatedAt).toLocaleString()}
                        disabled
                      />
                    </div>
                  </div>
