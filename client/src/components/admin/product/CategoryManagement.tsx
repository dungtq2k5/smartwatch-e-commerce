...
import {
  PRODUCT_CATEGORY_SORT_OPTIONS as CATEGORY_SORT_OPTIONS,
  PROJECT_NAME,
} from "../../../../../common/configs.common";
import type {
  AdminProductCategoryListResponse as CategoryListResponse,
  AdminProductCategoryResponse as CategoryResponse,
...
import {
  PRODUCT_CATEGORY_FIELD_LABEL_LEGEND as CATEGORY_FIELD_LABEL_LEGEND,
...
type Modal = {
  ...
  categoryIdsToDelete: string[] | null;
};

type TableColDisplay = {
  [key in AdminProductCategoryDisplayableField]: GeneralTableColDisplay<
    CategoryResponse,
    (typeof CATEGORY_SORT_OPTIONS)[number]
  >;
};
...
const SELECTION_TOAST_ID = "selected-categories-toast";
...
export default function CategoryManagement() {
  ...
  const {
    config: { productCategoryManagementDisplayFields: displayFields },
    resetProductCategoryManagementDisplayFields: resetDisplayFields,
    setProductCategoryManagementDisplayFields: setDisplayFields,
  } = useConfigStore();

  const [canEditCategory, canCreateCategory, canDeleteCategory, canReadUser] = [
    useHasPermission("u_product_cat"),
    useHasPermission("c_product_cat"),
    useHasPermission("d_product_cat"),
  ...
  const TABLE_COL_DISPLAY = useMemo(
    (): TableColDisplay => ({
      id: {
        label: CATEGORY_FIELD_LABEL_LEGEND["id"] || "ID",
        tdContent: (category) => <>{category.id}</>,
        getCsvVal: (category) => category.id,
      },
      name: {
        label: CATEGORY_FIELD_LABEL_LEGEND["name"] || "Name",
        isSortable: true,
        sortKey: { asc: "name_asc", desc: "name_desc" },
        tdContent: (category) => (
          <Link to={category.id} title="View detail category">
            {category.name}
          </Link>
        ),
        getCsvVal: (category) => category.name,
      },
      description: {
        label: CATEGORY_FIELD_LABEL_LEGEND["description"] || "Description",
        tdClassName: "td-description--truncate--g",
        tdContent: (category) => <>{category.description || "N/A"}</>,
        getCsvVal: (category) => category.description || "N/A",
      },
      createdBy: {
    ...
    [canDeleteCategory, canEditCategory, canReadUser],
  );
    ...
    const handleFetchSetInitialData = async () => {
      ...
      try {
        ...
        const newSearchForm: SearchForm = {
          ...
          sortBy: CATEGORY_SORT_OPTIONS.includes(
            urlSortBy as (typeof CATEGORY_SORT_OPTIONS)[number],
          )
            ? (urlSortBy as (typeof CATEGORY_SORT_OPTIONS)[number])
            : undefined,
        };
        ...
        setCategories(await fetchCategories(newSearchForm));
      } catch (error) {
      ...
      (t) => (
        ...
        >
          <div className="d-flex align-items-center gap-2">
            ...
            />
            <div className="fw-bold">{selectedCount} category(s) selected</div>
          </div>
  ...
  const handleSelectCategory = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>): void => {
      if (process.isProcessing || !categories) return;

      const { checked, name } = e.target;

      const categoryId = name.split("select-category-")[1];
      if (categoryId === "all") {
        setSelectedCategoryIds(checked ? "all" : []);
        return;
      }

      setSelectedCategoryIds((prev) => {
        let updatedSelectedCategoryIds: string[] | "all" = [];
        ...
        */
        ...
        if (prev === "all") {
          ...
          updatedSelectedCategoryIds = [...prev];
        ...
        return updatedSelectedCategoryIds.length === categories.categories.total
          ? "all"
          : updatedSelectedCategoryIds;
      });
    },
    [process.isProcessing, categories],
  );
  ...
  const genTable = useCallback((): JSX.Element => {
    ...
    const tableHeaders: JSX.Element[] = [
      <th key="th-select-all">
        <label htmlFor="select-category-all" hidden aria-hidden>
          Select all categories
        </label>
        <input
          type="checkbox"
          id="select-category-all"
          name="select-category-all"
          className="form-check-input"
          checked={selectedCategoryIds === "all"}
    ...
    const tableRows: JSX.Element = process.isFetching ? (
      <tr>
        <td colSpan={colSpan} className="p-4">
          <Loading loadingMsg="Searching categories..." />
        </td>
      </tr>
    ) : apiErr ? (
      <tr>
      ...
      <tr>
        <td colSpan={colSpan} className="p-4">
          <p className="mb-0 text-muted text-center">
            Uh oh! No categories found matching your criteria. Try adjust some
            filters or{" "}
            <button
              type="button"
  ...
  const handleApplyConfigDisplay = useCallback(
    (fields: CategoryDisplayField[]): void => {
  ...
  const handleExportList = useCallback(async (): Promise<void> => {
    ...
    if (!categories || categories.total === 0) {
      toast("No categories available to export.", { icon: WARNING_EMOJI });
      return;
    }
    ...
    try {
      // Fetch all categories matching the current filters, ignoring pagination
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const { limit, offset, ...exportQuery } = searchForm;
      const categoriesToExport = (
        await fetchCategories({
          ...exportQuery,
          limit: categories.total.toString(), // By default limit will be set to 9 at the BackEnd if not provided
        })
      ).categories;

      if (categoriesToExport.total === 0) {
        toast("No categories found to export.", { icon: WARNING_EMOJI });
        return;
      }
      ...
      const getVals = (
        ...
        return exportableFields.map((field) =>
          TABLE_COL_DISPLAY[field.name].getCsvVal(category),
        );
      };

      exportToCsv<CategoryResponse>(
        `${PROJECT_NAME.toLowerCase()}-categories-export-${new Date().toISOString()}.csv`,
        headers,
      ...
      toast.success(
        `Exported ${categoriesToExport.categories.length} categories successfully.`,
      );
    } catch (error) {
  ...
  const handleSubmitDeleteCategory = useCallback(async (): Promise<void> => {
    if (process.isProcessing) {
      toast("Another action is in progress. Please wait.", {
        icon: WAITING_EMOJI,
      });
    ...
    if (!modal.categoryIdToDelete) {
      toast.error("Category ID to delete not found.");
      return;
    }

    try {
      await deleteCategory(modal.categoryIdToDelete);
    ...
    canDeleteCategory,
    deleteCategory,
    modal.categoryIdToDelete,
    process.isProcessing,
    setSearchParams,
  ]);

  const handleSubmitDeleteCategoryBulk =
    useCallback(async (): Promise<void> => {
      ...
      if (
        !modal.categoryIdsToDelete ||
        modal.categoryIdsToDelete.length === 0
      ) {
        toast.error("No categories selected to delete.");
        return;
      }

      try {
        await deleteCategoryBulk({ categoryIds: modal.categoryIdsToDelete });
        toast.success(
          `${modal.categoryIdsToDelete.length} categories deleted successfully.`,
        );
  ...
  return (
    <>
          ...
          <div className="card-footer d-flex justify-content-end align-items-center gap-4 border-0">
            <div className="d-flex align-items-center gap-2">
              ...
              <select
                ...
                onChange={handleSearchChange}
                disabled={process.isProcessing || !categories}
              >
                {DATA_DISPLAY_ROWS_PER_PAGE.map((rowOption) => {
                  if (categories && categories.total < rowOption) return null;
            ...
            {categories && (
              <Pagination
                ...
                currentOffset={categories.offset}
      ...
      <ConfigDisplayModal
        ...
        legend={CATEGORY_FIELD_LABEL_LEGEND}
        onClose={closeModal}
      ...
      <ConfirmSubmitModal
        show={modal.categoryIdToDelete !== null}
        onHide={closeModal}
        onSubmit={handleSubmitDeleteCategory}
        custom={{
          action: "delete",
          title: `Delete category ID ${modal.categoryIdToDelete || "N/A"}`,
      ...
      <ConfirmSubmitModal
        show={modal.categoryIdsToDelete !== null}
        onHide={closeModal}
        onSubmit={handleSubmitDeleteCategoryBulk}
        custom={{
          action: "delete",
          title: `Delete selected categories (${
            modal.categoryIdsToDelete?.length || "N/A"
          })`,
          body: "Are you sure you want to delete all selected categories? This action cannot be undone.",
          cancelText: "Cancel",
          submitText: "Delete categories",
        }}
      />
    </>
  );
