...
import type { FormData } from "./EditOs";
...
import type { ProductOsCreate } from "../../../../../common/types.common";
...
export default function CreateOs() {
  ...
  console.log("CreateOs rendered", renderCount.current);

  const navigate = useNavigate();

  const { createOs } = useProductOsStore();

  const canCreateOs = useHasPermission("c_product_os");
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
      ...
      if (!canCreateOs) {
        toast.error("You do not have permission to create product OS.");
        return;
      }
      ...
      if (await validateForm()) {
        try {
          ...
          const os: ProductOsCreate = {
            name: formData.name.val,
            description: formData.description.val || null,
            logoUrl,
          };

          const createdOs = await createOs(os);
          navigate(`/admin/product-oses/${createdOs.id}`);
          toast.success("Product OS created successfully.");
        } catch (error) {
    ...
    [canCreateOs, createOs, formData, navigate, process.isProcessing],
  );
