...
import { USER_BANK_ACCOUNT_STATUS_LEGEND, WAITING_EMOJI } from "../../configs";
import toast from "react-hot-toast";
import SmallSpinner from "../SmallSpinner";
import { useUserBankAccountStore } from "../../store/userBankAccountStore";
...
export default function BankAndCard() {
  ...
  const handleDeleteCard = useCallback(async (): Promise<void> => {
    if (process.isProcessing) {
      toast("Another request is being processed. Please wait.", {
        icon: WAITING_EMOJI,
      });
  ...
  const handleCreateBankAccount = useCallback(async (): Promise<void> => {
    ...
    window.location.href = bankAccountSetup.setupUrl; // Redirect takes time
  ...
  const handleDeleteBankAccount = useCallback(async (): Promise<void> => {
    if (process.isProcessing) {
      toast("Another request is being processed. Please wait.", {
        icon: WAITING_EMOJI,
      });
