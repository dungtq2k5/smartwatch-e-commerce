...
const useUserBankAccountStore = create<UserBankAccountState>((set, get) => ({
  bankAccounts: null,
  ...
  async fetchBankAccounts(
    ...
    if (!oblige) {
      ...
      if (bankAccounts) return structuredClone(bankAccounts);
    }
    ...
    try {
      ...
      set({ bankAccounts });
      return structuredClone(bankAccounts);
    } catch (error) {
  ...
  async setDefaultBankAccount(
    ...
    try {
      ...
      return structuredClone(updatedBankAccount);
    } catch (error) {
      throw new Error(formatError(error));
    }
  },
