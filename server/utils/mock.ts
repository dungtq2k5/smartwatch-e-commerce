...
async function mockProductModels(
  ...
  try {
    ...
    for (const product of products) {
      const count = randNum(rand.min, rand.max);
      const usedWatchSizes = new Set<number>(); // Track used watch sizes to ensure uniqueness

      for (let i = 0; i < count; i++) {
        const stockPriceCents = faker.number.int({ min: 100_00, max: 1000_00 });

        let watchSizeMm: number;
        do {
          watchSizeMm = faker.number.int({ min: 30, max: 50 });
        } while (usedWatchSizes.has(watchSizeMm)); // Ensure unique watch size for each product
        usedWatchSizes.add(watchSizeMm);

        const model = {
          productId: product._id,
          model: `${faker.commerce.productName()} ${faker.string.alphanumeric(
            5
          )}`, // unique
          name: faker.commerce.productName(),
          // unique
          watchSizeMm: watchSizeMm,
          priceCents:
    ...
    for (const variation of modelVariations) {
      ...
      for (let i = 0; i < variation.stockQuantity; i++) {
        const instance = {
          ...
          supplierImeiNumber: isImeiUndefined ? undefined : faker.string.uuid(), // unique
